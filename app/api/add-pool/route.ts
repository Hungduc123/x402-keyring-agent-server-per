...
// The paid business logic for an add-pool (add-liquidity) action. Only runs after
// a valid x402 payment is verified by the facilitator, so reaching this line
// means the fee has been settled.
//
// The response body is a small JSON receipt: the client treats it as "the fee is
// paid, you may now broadcast the add-liquidity tx" — and nothing else. This
// endpoint does NOT touch the chain and never sees the user's transaction: the
// wallet builds, signs and broadcasts that itself.
...
// lib/x402.ts), keyed by "GET /api/add-pool" — this wrapper looks the request up
// there rather than taking payment requirements inline. Unpaid requests get HTTP
// 402 with those requirements; the client pays, retries with the payment header,
// and then receives the "ok" above.
//
